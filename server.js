        ...
        let jsonifyRes;
            try {
                ...
                messages: [
                    {
                    role: 'system',
                    content: `요리 순서 부분에 있는 문장에서 단위같은 부분들을 일상생활에서 활용하기 쉬운 예시로 바꿔줘. 예를들어 0.4cm는 잘게 이런 느낌으로. 근데 말투는 유지해줘야해. 요리순서 중 마지막 단계의 끝 부분에는 맛있게 드세요.를 추가해줘. 그 후에 텍스트를 아래와 같은 엄격한 JSON 스키마에 맞춰 변환해줘.
                                스키마:  
                                {  
                                "dish_name": string,  
